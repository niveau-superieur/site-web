...
<template>
  <div class="rounded-xl p-4 space-y-2 card">
    <div class="flex flex-col sm:flex-row sm:justify-between gap-1 items-center">
      <h2 class="text-heading mb-2">{{ stat.name }}</h2>
      <div class="">{{ stat.startDate }} - {{ stat.endDate }}</div>
    </div>

    <div class="flex flex-col sm:justify-end sm:flex-row">{{ stat.totalActivities }} activités</div>
    ...
    <div v-if="stat.highlights?.length">
      ...
      <h3 class="text-subheading mb-2">Activités marquantes</h3>
      ...
      <div class="flex flex-wrap flex-col gap-2">
        ...
        >
          ...
          <button
            ...
            class="px-2 py-1 rounded border hover-secondary"
